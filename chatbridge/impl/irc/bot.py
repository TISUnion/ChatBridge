...
class IRCBot(irc.client_aio.AioSimpleIRCClient):
	...
	def on_join(self, c, e):
		if e.source.nick == self.config.nickname:
			...
			stored.client.send_chat("{} joined the IRC".format(e.source.nick), author=e.source.nick)
	...
	def on_part(self, c, e):
		if e.target == self.config.channel:
			if e.arguments:
				...
				stored.client.send_chat("{} quited the IRC because of {}".format(e.source.nick, e.arguments[0]), author=e.source.nick)
			else:
				msg_debug = f'{e.source.nick} quited {e.target}'
				self.logger.info('Part: {}'.format(msg_debug))
				stored.client.send_chat("{} quited the IRC".format(e.source.nick), author=e.source.nick)
